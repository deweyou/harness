...
| 区域 | 位置 | 用途 |
|------|------|------|
| Skills | [`skills/`](/agents/skills/) | 会在特定 agent 任务中主动触发的工作流。 |
| Rules | [`rules/`](https://github.com/deweyou/agents/blob/main/rules/) | 跨项目复用的被动编码与开发偏好。 |
| Design | [`design/`](https://github.com/deweyou/agents/blob/main/design/) | 用于 AI 辅助 UI 工作的可复用界面设计契约。 |
| CLI | [`cli/`](https://github.com/deweyou/agents/blob/main/cli/) | `deweyou-cli` 二进制命令的 TypeScript 包。 |
| Docs | [`docs/`](https://github.com/deweyou/agents/blob/main/docs/) | 仓库工作流、设计记录和实现计划。 |
| Tests | [`tests/`](https://github.com/deweyou/agents/blob/main/tests/) | 资产注册表与扫描逻辑测试。 |

`AGENTS.md` 是给 agent 使用的导航页。仓库工作流细节记录在
[`docs/asset-workflow.md`](https://github.com/deweyou/agents/blob/main/docs/asset-workflow.md)。
...
| Skill | 介绍 | 来源 |
|-------|------|------|
| `repo-memory` | 仓库长期记忆工作流。它初始化和刷新 durable repo context，运行提交前记忆检查，在工作改变重要知识时更新文档，并检查本地 skill drift。 | [`skills/repo-memory/`](/agents/skills/repo-memory) |
| `git-delivery` | 分支感知的 git 交付工作流，覆盖分支准备、有意 staging、提交、base 分支冲突检查、安全 rebase、push、PR 创建、CI 跟进和明确低风险 CI 失败的自动修复。 | [`skills/git-delivery/`](/agents/skills/git-delivery) |
| `spec-driven-coding` | 面向功能、行为变更和多步骤实现的 spec-driven coding 工作流。它让 Superpowers spec、plan、TDD、验证和需求更新在编码前后保持一致。 | [`skills/spec-driven-coding/`](/agents/skills/spec-driven-coding) |
| `skill-eval` | 仓库本地的 skill 评测工作流。它可以生成 eval cases，通过 agent CLI 运行 routing 或 execution 测试，给 transcript 打分并汇总触发准确率。 | [`skills/skill-eval/`](/agents/skills/skill-eval) |
| `product-notes` | 产品笔记工作流，用于分类并沉淀产品想法、定位变化、迭代规格、决策、洞察和复盘。 | [`skills/product-notes/`](/agents/skills/product-notes) |
| `ui-design` | UX/UI 设计工作流，用于跨 Web、移动端、HarmonyOS、小程序、macOS、仪表盘和工具进行模式调研、流程设计、视觉风格、实现、审查和 AI 设计 prompt 生成。 | [`skills/ui-design/`](/agents/skills/ui-design) |
| `product-design` | 面向个人产品的产品设计工作流。它在需要时调研现有产品，避免企业级流程表演，并给出合适深度的方向、版本或验证步骤建议。 | [`skills/product-design/`](/agents/skills/product-design) |
...
| Rule | 介绍 | 来源 |
|------|------|------|
| `collaboration-defaults` | 默认 agent 协作行为，覆盖语言、歧义、上下文、任务顺序、并行工作、证据、安全和交付。 | [`rules/collaboration-defaults.md`](https://github.com/deweyou/agents/blob/main/rules/collaboration-defaults.md) |
| `code-style` | 命名、函数、注释、错误处理和测试的局部代码表达偏好。 | [`rules/code-style.md`](https://github.com/deweyou/agents/blob/main/rules/code-style.md) |
| `engineering-principles` | 模块边界、抽象、依赖、状态和易删除代码的设计偏好。 | [`rules/engineering-principles.md`](https://github.com/deweyou/agents/blob/main/rules/engineering-principles.md) |

## Design

设计契约在这个资产中心里放在 [`design/`](https://github.com/deweyou/agents/blob/main/design/) 下。它们是项目级设计契约：
一部分是设计规则，一部分是 token map，一部分是组件指导。`ui-design` 会优先读取
项目本地的 `DESIGN.md`；`deweyou-cli agent init --design dewey-interface`
会把 [`design/dewey-interface.md`](https://github.com/deweyou/agents/blob/main/design/dewey-interface.md) 安装到目标仓库根目录为
`DESIGN.md`。

| Design Contract | 介绍 | 来源 |
|-----------------|------|------|
| `dewey-interface` | 面向个人产品的克制、排版驱动、组件化界面风格。 | [`design/dewey-interface.md`](https://github.com/deweyou/agents/blob/main/design/dewey-interface.md) |
