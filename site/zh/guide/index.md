...
[`docs/asset-workflow.md`](https://github.com/deweyou/agents/blob/main/docs/asset-workflow.md)。DDev 技术方案和日常操作
手册分别记录在 [`docs/ddev-framework.zh.md`](/zh/ddev/framework) 和
...
| `ddev-local-state` | `~/.deweyou/dev` 下全局 DDev 本地状态的归属、可见性、清理和提交边界。 | [`rules/ddev-local-state.md`](https://github.com/deweyou/agents/blob/main/rules/ddev-local-state.md) |
| `verification-evidence` | 完成声明、跳过检查、UI/runtime 现场证明和验证缺口的证据要求。 | [`rules/verification-evidence.md`](https://github.com/deweyou/agents/blob/main/rules/verification-evidence.md) |
| `loop-boundaries` | 约束实现、调试、验证和 CI 修复循环，明确什么时候继续、停止或提问。 | [`rules/loop-boundaries.md`](https://github.com/deweyou/agents/blob/main/rules/loop-boundaries.md) |
