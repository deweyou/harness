...
export async function checkDoctor({
  ...
  const cacheManifest = await readJsonCheck({
    path: paths.manifestPath,
    checks,
    passMessage: 'local asset cache manifest exists',
    missingMessage: 'Dewey asset cache manifest is missing. Run `deweyou-cli agent update`.',
    invalidMessage: 'Dewey asset cache manifest is invalid JSON.',
  })
  ...
  if (cacheManifest !== undefined) validateCacheManifest(cacheManifest, checks)
  const manifestValid =
...
function validateCacheManifest(
  manifest: unknown,
  checks: DoctorCheck[],
): manifest is CacheManifest {
  if (!isPlainObject(manifest)) {
    checks.push(fail('asset cache manifest must be an object'))
    return false
  }
  let valid = true
  if (manifest.cliVersion !== CLI_VERSION) {
    checks.push(fail(
      `asset cache was generated by deweyou-cli ${String(manifest.cliVersion)}, current ${CLI_VERSION}. Run \`deweyou-cli agent update\`.`,
    ))
    valid = false
  }
  const capabilities = Array.isArray(manifest.capabilities)
    ...
    : []
  const missing = CLI_CAPABILITIES.filter((capability) => !capabilities.includes(capability))
  if (missing.length > 0) {
    checks.push(fail(
      `asset cache is missing CLI capabilities: ${missing.join(', ')}. Run \`deweyou-cli agent update\`.`,
    ))
    valid = false
  }
  if (valid) checks.push(pass('asset cache CLI version and capabilities are compatible'))
  return valid
}
