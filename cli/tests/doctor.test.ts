...
describe('checkDoctor', () => {
  ...
  it('fails when cache CLI version or capabilities do not match', async () => {
    const { homeDir, repoRoot } = await createDoctorFixture({ mode: 'link' })
    const path = join(homeDir, '.deweyou/agents/manifest.json')
    const manifest = JSON.parse(await readFile(path, 'utf8'))
    manifest.cliVersion = '0.1.0'
    ...
    assert.equal(result.ok, false)
    assert.match(failMessages(result), /generated by deweyou-cli 0\.1\.0/)
    assert.match(failMessages(result), /missing CLI capabilities/)
  })
